...
<aside class="w-[300px] bg-pink-50 rounded-2xl border border-pink-100 p-6
              flex flex-col justify-between items-center min-h-[600px] shadow-sm">
    ...
    <nav class="mt-6 space-y-3 w-full">
        ...
        {{-- Orders (เดิมเขียนว่า Purchase order) --}}
        ...
        {{-- Address (ตัวอย่าง/ที่ว่างไว้) --}}
        <a href="#"
           class="{{ $linkBase }} text-gray-800 hover:bg-pink-100">
            <span class="{{ $iconWrap }}">
                <svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
    ...
    {{-- Logout --}}
    <form method="POST" action="{{ route('logout') }}" class="mt-auto w-full text-center">
        @csrf
        <button type="submit"
            class="mt-8 bg-pink-200 hover:bg-pink-300 text-gray-800 font-semibold px-8 py-2 rounded-full shadow transition">
            Logout
        </button>
    </form>
</aside>
